...
namespace Microsoft.BotBuilderSamples
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            if (!configuration.GetValue<string>("BLOB_CONNECTION_STRING").IsNullOrEmpty())
                if (!configuration.GetValue<string>("BLOB_CONNECTION_STRING").IsNullOrEmpty())
                    services.AddSingleton(new BlobServiceClient(configuration.GetValue<string>("BLOB_CONNECTION_STRING")));
